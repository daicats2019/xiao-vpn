...
import 'package:xiao_vpn/domain/model/index.dart';

import '../../data/sharedpref/shared_prefs.dart';
...
class AppCubit extends Cubit<AppState> {
  ...
  Future<void> fetchServerList({bool isRefresh = false}) async {
    ...
    currentServer ??= result.firstOrNull;
    emit(
      state.copyWith(
        servers: result,
        isLoading: false,
  ...
  void _doConnect(int id, String name, String config) {
    Utils.log(_tag, 'connect: id = $id, config = $config');
...
//region Billing
  ...
  Future<void> startBilling() async {
    ...
    final items = await _getProductDetails();
    Utils.log(_tag, 'items: ${items.length}');
    emit(
      state.copyWith(
        productDetailsList: items,
        selectedProductDetails: items.firstOrNull,
      ),
    );
  }

  void setProductDetails(ProductDetails productDetails) {
    emit(state.copyWith(selectedProductDetails: productDetails));
  }

  Future<void> subscribe() async {
    final productDetails = state.selectedProductDetails;
  ...
  Future<List<ProductDetails>> _getProductDetails() async {
    final response = await _inAppPurchase.queryProductDetails(Config.subIds);
